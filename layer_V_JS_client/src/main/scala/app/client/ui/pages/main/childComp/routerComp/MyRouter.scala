...
import app.client.ui.pages.main.childComp.routerComp.childOfRouter.navigator.childOfNavigator.possibleChildrenOfNavigator_depth1Components.lineDetail.LineDetail_CompConstr_Holder.F
...
//        val f: D2toD1Transformer => (LineDetail_URL, RouterCtl[URL_STr]) => Res =
        val f: D2toD1Transformer => (LineDetail_URL, RouterCtl[URL_STr]) => F.Res =
        {

          val r: D2toD1Transformer => F.Res2 = LineDetail_CompConstr_Holder.getCompConstr_For_dynRenderR_In_Router
          ???
        }

        val h: (LineDetail_URL, RouterCtl[URL_STr]) => F.Res = f(trD2toD1)
//        val h: F.Par  => F.Res = f(trD2toD1)
