...
class JSDOMNodeJSEnv2(@deprecatedName('nodejsPath) executable: String = "node",
                      @deprecatedName('addArgs) args: Seq[String] = Seq.empty,
                      @deprecatedName('addEnv) env: Map[String, String] =
                        Map.empty)
  ...
  protected class DOMNodeRunner(libs: Seq[ResolvedJSDependency],
                                code: VirtualJSFile)
      extends ExtRunner(libs, code)
      with AbstractDOMNodeRunner

  protected class AsyncDOMNodeRunner(libs: Seq[ResolvedJSDependency],
                                     code: VirtualJSFile)
      extends AsyncExtRunner(libs, code)
      with AbstractDOMNodeRunner

  protected class ComDOMNodeRunner(libs: Seq[ResolvedJSDependency],
                                   code: VirtualJSFile)
      extends AsyncDOMNodeRunner(libs, code)
      with NodeComJSRunner
  ...
  protected trait AbstractDOMNodeRunner extends AbstractNodeRunner {
    ...
    protected def codeWithJSDOMContext(): Seq[VirtualJSFile] = {
      ...
      Seq(
        new MemVirtualJSFile("codeWithJSDOMContext.js").withContent(jsDOMCode))
    }
